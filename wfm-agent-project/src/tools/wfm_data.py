...
def compare_two_days(df: pd.DataFrame, language: str, lob: str, date1: str, date2: str) -> dict:
    """Compares metrics and service level between two days.
    ...
    Returns:
        A dict with both days' raw metrics, plus the differences
    """
...
def _calculate_forecast_from_filtered(filtered: pd.DataFrame, target_volume: int) -> dict:
    """Calculates forecast distribution from already-filtered data.

    Args:
        filtered: DataFrame already filtered by language, LOB, and date(s)
    ...
    total_offered = filtered["offered"].sum()

    result = {}
    for interval, offered in total_offered_per_interval.items():
        fraction = offered / total_offered
...
def forecast_by_pattern(df: pd.DataFrame, language: str, lob: str, historical_date: str, target_volume: int) -> dict:
    ...
    a single historical day's pattern.
    ...
    return _calculate_forecast_from_filtered(filtered, target_volume)
...
if __name__ == "__main__":
    ...
    print(get_daily_metrics(df, "Language 1", "LOB 1", "2015-10-20"))
    print(calculate_service_level(df, "Language 1", "LOB 1", "2015-10-20"))
    print(get_talktime_by_period(df, "Language 1", "LOB 1", "2015-10-14", "2015-10-20"))
    print(get_monthly_distribution_by_language(df, "LOB 1"))
